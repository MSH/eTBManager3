 ...
 */
export function validateForm(elems, dm) {
	let errors = null;

	elems.forEach(elem => {
		if (elem.el === 'field') {
			const typeHandler = Types.list[elem.type];
			const value = getValue(dm, elem.property);
			...
			if (isEmpty(value)) {
				...
				// validate value
				msg = typeHandler.validate(elem, value, dm);
			}
