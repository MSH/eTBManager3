...
module.exports = function execute(opt) {
    ...
    var crud = require('../commons/crud')(opt.name);

    var doc = opt.doc;

    // create test
    it('# create', () => {
        return crud.create(opt.doc)
        .then(res => {
            if (opt.afterCreate) {
        ...
        it('# required fields', () => {
            ...
            .then(res => {
                ...
                res.errors.forEach(msg => {
                    assert.equal(msg.code, 'NotNull');
                    assert(opt.requiredFields.indexOf(msg.field) >= 0);
                });
            });
        });
    }
    ...
    if (opt.uniqueFields) {
        it('# unique', () => {
            var proms = opt.uniqueFields.map(group => {
                var doc2 = Object.assign({}, opt.doc);
    ...
    if (opt.update) {
        it('# update', function() {
            ...
            if (opt.update.set) {
                ...
                return crud.update(doc2.id, set)
                .then(() => {
                    return crud.findOne(doc2.id);
                })
                .then(res => {
                    assert.equal(res.id, doc2.id);
                    for (var key in set) {
            ...
            if (opt.update.fields) {
                ...
                return crud.update(doc2.id, data)
                .then(() => crud.findOne(doc2.id))
                .then(res => {
                    assert.equal(res.id, doc2.id);
                    for (var key in data) {
